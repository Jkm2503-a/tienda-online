...
class Pedido{
    ...
    public function registrarDetalle($_params){
        $sql = "INSERT INTO `detalle_pedidos`(`pedido_id`, `productos_id`, `precio`, `cantidad`) 
        VALUES (:pedido_id,:productos_id,:precio,:cantidad)";
 ...
 //--------------------------------------------------------------------------//   
    ...
    {
        $sql = "SELECT p.id, nombre, apellidos, cedula, email, total, fecha FROM pedidos p 
    ...
    public function mostrarDetallePorIdPedido($id)
    {
        $sql = "SELECT 
                ...
                INNER JOIN productos pe ON pe.id= dp.productos_id
